// ==========================================================
// Mock Top-X% Pipeline  --  V0.7.2
// Author 	: Kolja Hildenbrand
// Date   	: 2026-06-02
// Status	: OLD
//
// Changes vs V0.6.1:
//  - CHANGE: The fixed, global ARTIFACT_UPPER_BOUND is replaced by an
//            ADAPTIVE per-channel artifact bound. An artifact is a pixel
//            sitting far above THAT channel's own autofluorescence.
//            V0.7.2 uses a robust "centre + k*spread" fence per channel:
//                thr = mean_bulk + ARTIFACT_K * std_bulk
//            where mean_bulk/std_bulk are over the cytosol pixels > 0 with
//            the top ARTIFACT_TRIM_PCT % excluded (so the artifacts cannot
//            inflate the std meant to detect them). It only cuts when a
//            SEPARATED bright population exists: max > ARTIFACT_SEP * thr;
//            otherwise nothing is cut (artifact-free images stay intact).
//            (V0.7.0 tried a Q3+K*IQR fence — too low, sat in the dim bulk;
//            V0.7.1 used MULT*p99.9. v0.7.2 is the user's mean+std idea,
//            made robust by trimming + the separation gate.)
//            ARTIFACT_K is the knob; larger = more conservative.
//  - NEW:    computeArtifactBound() prints the per-image, per-channel
//            decision (mean_b / std_b / thr / max -> bound) to the Log.
//  - CSV:    column `artifact_upper_bound` -> `artifact_bound` (per-channel
//            value actually used) + column `artifact_k`.
//
// ==========================================================
...
// ARTIFACT EXCLUSION — ADAPTIVE per-channel bound (V0.7.2)
// An artifact (dust, debris) is a pixel FAR ABOVE that channel's own
// autofluorescence. Per image+channel we build a "centre + k*spread" fence
// from the channel's CYTOSOL pixels, made robust + gated:
//   1. BULK statistics: mean & std over the cytosol pixels > 0, EXCLUDING the
//      top ARTIFACT_TRIM_PCT % (the suspected artifacts). Trimming stops the
//      artifacts from inflating the very std meant to detect them (the
//      "masking" effect), so the fence is stable image-to-image.
//   2. Threshold:  thr = mean_bulk + ARTIFACT_K * std_bulk.
//   3. SEPARATION GATE: only cut if the brightest pixel is clearly separated
//      from the fence (max > ARTIFACT_SEP * thr) — i.e. a real, distinct dust
//      population. Otherwise the bright tail is just autofluorescence -> no cut
//      (so artifact-free images are never trimmed).
// Note: trimming makes std_bulk UNDERESTIMATE the autofluorescence spread, so
// thr can land inside the real tail — the separation gate is what protects
// clean images. ARTIFACT_K is the knob to lift thr clear of autofluorescence:
//   larger  = more conservative (higher thr, fewer/no cuts),
//   smaller = more aggressive (cuts closer to the autofluorescence top).
// Per-channel bound + decision are logged and written to the CSV.
var ARTIFACT_K         = 100.0;        // std multiplier (the knob)
var ARTIFACT_SEP       = 2.0;        // separation guard: cut only if max > SEP*thr
var ARTIFACT_TRIM_PCT  = 1.0;        // top % excluded from the bulk mean/std
...
var CSV_HEADER = "image,cell_line,timepoint,combo,channel,"
               ...
               + "artifact_bound,artifact_k,min_particle_size," /* V0.4; artifact_bound adaptive in V0.7 */
...
print("Artifact fence: thr = mean_bulk + ARTIFACT_K*std_bulk (cut if max>SEP*thr), K = " + ARTIFACT_K + ", SEP = " + ARTIFACT_SEP);
...
function applyCellLineDefaults(cellLine) {
    ...
    print("  ARTIFACT_K        = " + ARTIFACT_K + " (mean_bulk + K*std_bulk, SEP=" + ARTIFACT_SEP + ")");
...
function processOneImage(fname) {
    ...
    // ---- V0.7: adaptive per-channel artifact bound -------
    // Compute each channel's bound from its OWN cytosol pixel distribution
    // (Cytosol_Mask still raw here, before any artifact removal). These are
    // logged, used for cleanup, and written to the CSV per channel.
    artifactBound1 = computeArtifactBound(m1 + "_channel", m1, "C1");
    artifactBound2 = computeArtifactBound(m2 + "_channel", m2, "C2");
    ...
    nArtifactsExcluded = cleanCytosolMask(m1, m2, artifactBound1, artifactBound2);
    ...
    // ---- measure each marker ------------------------------
    measureAndWrite(m1, m1 + "_channel", cytoRoiId, nCyto, nArtifactsExcluded, artifactBound1, imgName, cellLine, tp, comboKey);
    measureAndWrite(m2, m2 + "_channel", cytoRoiId, nCyto, nArtifactsExcluded, artifactBound2, imgName, cellLine, tp, comboKey);
...
// Adaptive per-channel artifact bound, computed from the channel's CYTOSOL
// pixels (Cytosol_Mask must already exist). Robust "centre + k*spread" fence:
//   thr = mean_bulk + ARTIFACT_K * std_bulk   (bulk = pixels>0 minus top TRIM%)
// only applied when a separated bright population exists (max > SEP*thr).
// Prints the decision so it is auditable in the Log.
// Returns the bound (> 65535 = cut nothing).
function computeArtifactBound(chTitle, marker, chLabel) {
    if (!isOpen(chTitle)) return 65536;
    // Guard: an all-background cytosol would make Create Selection throw.
    if (countMaskForeground("Cytosol_Mask") == 0) {
        print("  artifact bound  " + chLabel + " " + marker + " : empty cytosol -> 65536 (cut nothing)");
        return 65536;
    }
    // Restrict the channel to the current cytosol region.
    selectWindow("Cytosol_Mask");
    run("Create Selection");
    ...
    run("Select None");
    // Clear the helper selection on the mask too, so later whole-mask
    // histograms (countMaskForeground) are not silently selection-restricted.
    ...
    // --- BULK mean & std over pixels > 0, excluding the top TRIM% ----------
    // Trimming the top (suspected artifacts) keeps them from inflating the std
    // that is meant to detect them (the masking effect). pTrim = upper edge of
    // the bulk; bins above it are ignored for the centre/spread estimate.
    pTrim = pctIndex(counts, nBins, nTotal, 100.0 - ARTIFACT_TRIM_PCT);
    nBulk = 0; sumV = 0; sumV2 = 0;
    for (i = 1; i <= pTrim; i++) {          // i=1 -> exclude the 0 bin
        if (counts[i] > 0) {
            nBulk += counts[i];
            sumV  += i * counts[i];
            sumV2 += i * i * counts[i];
    ...
    if (nBulk == 0) {
        print("  artifact bound  " + chLabel + " " + marker + " : empty bulk -> no cut");
        return 100000;
    }
    meanBulk = sumV / nBulk;
    varBulk  = (sumV2 / nBulk) - meanBulk * meanBulk;
    stdBulk  = sqrt(maxOf(varBulk, 0));

    // Brightest occupied intensity in the cytosol.
    pMax = 0;
    for (i = nBins - 1; i >= 0; i--) {
        if (counts[i] > 0) { pMax = i; break; }
    }

    // Threshold = centre + k*spread of the bulk.
    thr = meanBulk + ARTIFACT_K * stdBulk;

    // SEPARATION GATE: only cut if the brightest pixel is clearly separated
    // from the fence (a distinct dust population). Otherwise the bright tail is
    // just autofluorescence -> cut NOTHING.
    bound = 100000;                 // sentinel > 65535 = "no cut"
    decision = "no artifacts (max <= SEP*thr)";
    if (thr > 0 && pMax > ARTIFACT_SEP * thr) { bound = round(thr); decision = "CUT above"; }

    print("  artifact bound  " + chLabel + " " + marker
        + " : mean_b=" + d2s(meanBulk, 1) + " std_b=" + d2s(stdBulk, 1)
        + " K=" + ARTIFACT_K + " thr=" + d2s(thr, 1) + " max=" + pMax
        + " | sep@ SEP(" + ARTIFACT_SEP + ")xthr=" + d2s(ARTIFACT_SEP * thr, 1)
        + " -> " + decision + " (bound=" + bound + ")");
    return bound;
}

function makeArtifactMaskFor(srcTitle, maskName, bound) {
    selectWindow(srcTitle);
    run("Select None");
    run("Duplicate...", "title=" + maskName);
    if (bound > 65535) {
        // "no artifacts" -> empty mask (zero everything, then binarise).
        run("Multiply...", "value=0");
        setThreshold(1, 65535);
        run("Convert to Mask");
        return;
    }
    setThreshold(bound, 65535);
    run("Convert to Mask");
...
function buildCombinedArtifactMask(m1, m2, b1, b2) {
    makeArtifactMaskFor(m1 + "_channel", "Artifact_M1", b1);
    makeArtifactMaskFor(m2 + "_channel", "Artifact_M2", b2);
...
function cleanCytosolMask(m1, m2, b1, b2) {
    // Count cytosol pixels BEFORE cleanup
    nBefore = countMaskForeground("Cytosol_Mask");

    // (1+2+3) Artifact subtraction (per-channel adaptive bounds b1, b2)
    buildCombinedArtifactMask(m1, m2, b1, b2);
    ...
    print("  artifact excl: " + nArtifactsExcluded + " px removed (bounds " + m1 + ">" + b1 + ", " + m2 + ">" + b2 + ")");
...
function measureAndWrite(marker, chTitle, cytoRoiId, nCyto, nArtifactsExcluded, artifactBound,
    ...
    appendCsvRow(csvPath, imgName, cellLine, tp, comboKey, marker, stats, nCyto, nArtifactsExcluded, artifactBound);
}
...
function appendCsvRow(csvPath, imgName, cellLine, tp, comboKey, channel,
                      stats, nCyto, nArtifactsExcluded, artifactBound) {
    line = imgName + "," + cellLine + "," + tp + "," + comboKey + "," + channel
        ...
        + "," + BLUR_SIGMA_CELL + "," + BLUR_SIGMA_NUC
        + "," + artifactBound + "," + ARTIFACT_K + "," + MIN_PARTICLE_SIZE
        + "," + MACRO_VERSION + "," + RUN_ID;
