...
//            V0.7.1 anchors on the TOP of the autofluorescence (its 99.9th
//            percentile) and only cuts when a SEPARATED bright population
//            exists:  cut = ARTIFACT_MULT * p99.9;  cut only if max > cut.
//            (V0.7.0 used a Q3+K*IQR fence, but the cytosol is mostly dim,
//            so the quartiles sat in the dark bulk and the fence cut real
//            signal — and clean images got cut too. Anchoring on p99.9
//            fixes both.) ARTIFACT_MULT is the single knob (default 3.0).
//            Fixes: dsRNA's high autofluorescence was being cut by the old
//            fixed bound (deflating its background), while a higher fixed
//            bound let dust survive in the dimmer HA / NS4B channels.
//  - NEW:    computeArtifactBound() prints the per-image, per-channel
//            decision (p50 / p99.9 / max / cut -> bound) to the Log.
//  - CSV:    column `artifact_upper_bound` -> `artifact_bound` (per-channel
//            value actually used) + new column `artifact_mult`.
//
// ==========================================================
...
// ARTIFACT EXCLUSION — ADAPTIVE per-channel bound (V0.7.1)
// An artifact (dust, fluorescent debris) is a pixel FAR ABOVE that channel's
// own autofluorescence. The cytosol is mostly DIM with a sparse bright tail,
// so an IQR/quartile fence (V0.7.0) sat in the dark bulk and cut real signal.
// Instead we anchor on the TOP of the autofluorescence — its 99.9th
// percentile (robust: dust is < 0.1 % of cytosol pixels) — and only cut when
// a SEPARATED bright population exists:
//     cut = ARTIFACT_MULT * p99.9
//     if (max_intensity > cut)  -> bound = cut         (artifacts present)
//     else                      -> no cut              (just autofluorescence)
//
// ARTIFACT_MULT is the knob: "a pixel is an artifact if it is more than
// MULT x brighter than the 99.9th percentile of the channel."
//   larger  = more conservative (only very-separated dust cut; clean images
//             and the measured top-pool stay safe),
...
var ARTIFACT_MULT        = 3.0;       // multiplier on the p99.9 anchor (the knob)
var ARTIFACT_ANCHOR_PCTL = 99.9;     // autofluorescence-top anchor percentile
...
var CSV_HEADER = "image,cell_line,timepoint,combo,channel,"
               ...
               + "artifact_bound,artifact_mult,min_particle_size," /* V0.4; artifact_bound adaptive in V0.7 */
...
print("Artifact fence: cut = ARTIFACT_MULT*p99.9 (only if max>cut), ARTIFACT_MULT = " + ARTIFACT_MULT);
...
function applyCellLineDefaults(cellLine) {
    ...
    print("  ARTIFACT_MULT     = " + ARTIFACT_MULT + " (x p99.9, adaptive per-channel)");
...
// pixels (Cytosol_Mask must already exist). Anchors on the autofluorescence
// top (p99.9) and only cuts when a separated bright population exists:
//   cut = ARTIFACT_MULT * p99.9;  bound = cut if max > cut, else no cut.
...
function computeArtifactBound(chTitle, marker, chLabel) {
    ...
    // Anchor on the TOP of the autofluorescence (its 99.9th percentile), NOT
    // on the quartiles: the cytosol is mostly dim, so Q1/Q3 sit in the dark
    // bulk and an IQR fence lands far below the real signal. p99.9 is robust
    // to the (rare, <0.1 %) artifact pixels themselves.
    pMed = pctIndex(counts, nBins, nTotal, 50);
    pHi  = pctIndex(counts, nBins, nTotal, ARTIFACT_ANCHOR_PCTL);   // ~99.9
    ...
    // An artifact is a SEPARATED bright population: the max must exceed
    // ARTIFACT_MULT x the autofluorescence top. If it does not, the bright
    // tail is just autofluorescence -> cut NOTHING (bound above 16-bit max).
    cut   = ARTIFACT_MULT * pHi;
    bound = 100000;                 // sentinel > 65535 = "no cut"
    decision = "no artifacts (max <= cut)";
    if (pHi > 0 && pMax > cut) { bound = cut; decision = "CUT above"; }

    print("  artifact bound  " + chLabel + " " + marker
        + " : p50=" + pMed + " p99.9=" + pHi + " max=" + pMax
        + " | cut@ MULT(" + ARTIFACT_MULT + ")xp99.9 = " + cut
...
function appendCsvRow(csvPath, imgName, cellLine, tp, comboKey, channel,
    ...
    line = imgName + "," + cellLine + "," + tp + "," + comboKey + "," + channel
        ...
        + "," + BLUR_SIGMA_CELL + "," + BLUR_SIGMA_NUC
        + "," + artifactBound + "," + ARTIFACT_MULT + "," + MIN_PARTICLE_SIZE
        + "," + MACRO_VERSION + "," + RUN_ID;
