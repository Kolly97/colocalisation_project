...
//            sitting far above THAT channel's own autofluorescence, so the
//            bound is now derived per image+channel from the channel's
//            cytosol pixels as a robust Tukey upper outlier fence:
//                bound = Q3 + ARTIFACT_K * (Q3 - Q1)
//            ARTIFACT_K is the single strictness knob (default 3.0).
//            Fixes: dsRNA's high autofluorescence was being cut by the
...
//            threshold (Q1/Q3/IQR/K -> bound) to the Log.
//  - CSV:    column `artifact_upper_bound` -> `artifact_bound` (per-channel
//            value actually used) + new column `artifact_k`.
//
// Changes vs V0.5:
...
// ARTIFACT EXCLUSION — ADAPTIVE per-channel bound (V0.7)
// An artifact (dust, fluorescent debris) is a pixel sitting FAR ABOVE that
// channel's own autofluorescence — not at some fixed absolute intensity.
// So the bound is computed per image+channel from the channel's CYTOSOL
// pixels as a robust Tukey upper outlier fence:
//     bound = Q3 + ARTIFACT_K * (Q3 - Q1)        (Q1/Q3 = 25th/75th pct)
// Q1/Q3 are robust to the few artifact pixels themselves (dust << 25 % of
// the cytosol), so the fence is set by the autofluorescence bulk.
//
// ARTIFACT_K is the ONLY knob:
//   larger  = more conservative (only clearly separated dust is cut; safer
//             for the measured top-pool, but borderline debris may survive),
//   smaller = more aggressive (cuts more, but risks trimming the bright
//             autofluorescence tail we actually measure).
// When a channel has no artifacts the fence sits above every real pixel and
// nothing is cut. Per-channel bounds are logged and written to the CSV.
var ARTIFACT_K           = 10.0;      // Tukey "far outlier" fence multiplier
...
print("Artifact fence: bound = Q3 + ARTIFACT_K*(Q3-Q1), ARTIFACT_K = " + ARTIFACT_K);
...
function applyCellLineDefaults(cellLine) {
    ...
    print("  ARTIFACT_K        = " + ARTIFACT_K + " (adaptive per-channel bound)");
...
// Robust per-channel artifact bound = Q3 + ARTIFACT_K*(Q3-Q1), computed from
// the channel's CYTOSOL pixels (Cytosol_Mask must already exist). Prints the
// inputs so the per-image, per-channel threshold is auditable in the Log.
// Returns the bound (65536 = degenerate -> cut nothing).
function computeArtifactBound(chTitle, marker, chLabel) {
    ...
    q1  = pctIndex(counts, nBins, nTotal, 25);
    q3  = pctIndex(counts, nBins, nTotal, 75);
    iqr = q3 - q1;
    bound = q3 + ARTIFACT_K * iqr;
    if (iqr <= 0) bound = 65536;   // flat region -> don't over-trim
    print("  artifact bound  " + chLabel + " " + marker
        + " : Q1=" + q1 + " Q3=" + q3 + " IQR=" + iqr + " K=" + ARTIFACT_K + " -> " + bound);
    return bound;
}
