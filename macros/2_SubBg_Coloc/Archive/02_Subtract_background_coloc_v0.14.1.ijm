// ==========================================================
// Background Subtraction + Coloc Prep Pipeline  --  V0.14.1
...
//
// Changes vs V0.14:
//	CHANGED:	ROI area has to be over 80000 px^2
//	CHANGED: 	Percentille measured around nucleus changed from 99.9 -> 99.99
//
...
//    startup (askManualRoiOptions / global MANUAL_EXCLUDE_NUC), not per image.
//
...
var CELL_MIN_SIZE   = 80000;        // px, min area of a KEPT per-cell ROI
...
var SIGNAL_BG_MULT = 2;       // "real signal >= 2x the Mock background"
var GATE_RING_UM   = 4;       // microns the nucleus is grown to sample cytoplasm
...
// image loop). Sets the global MANUAL_EXCLUDE_NUC.
// ------------------------------
function askManualRoiOptions() {
    ...
    Dialog.addCheckbox("Exclude nucleus from drawn ROIs (auto, via DAPI mask)", MANUAL_EXCLUDE_NUC);
    Dialog.show();
    MANUAL_EXCLUDE_NUC = Dialog.getCheckbox();
    nucLabel = "kept in ROI";
    if (MANUAL_EXCLUDE_NUC) nucLabel = "subtracted from ROI";
