// ==========================================================
// Background Subtraction + Coloc Prep Pipeline  --  V0.2
...
//
// Changes vs V0.1.0:
//  - NEW: Cytosol ROI. V0.1.0 only subtracted the background; V0.2 builds a
//         cytosol ROI (cell mask − nucleus mask) from the cell-mask source
//         channel on the RAW signal, BEFORE subtraction, so the threshold sees
//         real signal.
//  - NEW: Re-merge into a scrollable Color hyperstack (like the original CZI),
//         saved as 16-bit TIF.
//  - NEW: Colocalisation hand-off — pause for the user to run Coloc 2 on the two
//         non-DAPI channels inside the ROI; append a PLACEHOLDER row to a per-run
//         coloc CSV (the real Coloc -> CSV bridge is planned for V0.4).
//  - (Still no particle-size filter, cell-line defaults, or QC JPG — those come
//    in V0.5+.)
//
