// ==========================================================
// Background Subtraction + Coloc Prep Pipeline  --  V0.12
// Author 	: Kolja Hildenbrand
// Date   	: 2026-06-03
// Status	: OLD (superseded by v0.14.3)
//
// Changes vs V0.11:
//  AUTOMATIC ROI — "grow each nucleus inside the cell mask":
//   - V0.11 cut the cell mask by Voronoi ridges then ran Analyze Particles on
//     the pieces → on real (patchy / confluent) data this produced many tiny
//     fragments that are not cells, and most images kept 0.
//   - V0.12 keeps the Voronoi cut (the between-cell boundaries) but then GROWS
//     EACH NUCLEUS: it flood-fills (doWand) the cell territory starting at the
//     nucleus centroid. So every ROI is anchored to exactly one nucleus
//     (no nucleus-less fragments), an isolated nucleus grabs its whole mask blob,
//     and touching cells stop at the ridge between them — matching the
//     hand-drawn cell separation.
//   - Cell mask is OR'd with the nuclei so every nucleus is a valid seed.
//   - Keep rule unchanged (area > CELL_MIN_SIZE px AND raw p99 >= SIGNAL_BG_MULT
//     x bg in BOTH channels). Optional nucleus subtraction (AUTO_EXCLUDE_NUC).
//
// Changes vs V0.10:
...
function logHeader(nFiles) {
    ...
    if (ROI_MODE == "auto_central") {
        print("auto segmentation : grow each nucleus in cell mask (Voronoi cut + flood-fill), nuclei >= "
...
// ------------------------------
// AUTOMATIC (V0.12): grow each nucleus inside the cell mask until it hits the
// mask edge (isolated cell) or a neighbour (touching cells).
//
//   1. Cell mask (solid) OR nuclei  -> where cells are; nuclei always included.
//   2. Cut it with the nucleus-Voronoi ridges (the watershed boundaries between
//      neighbouring cells)  -> Cells_Split.
//   3. For EACH nucleus, flood-fill (doWand) its cell territory from the nucleus
//      centroid. So every ROI is anchored to a nucleus (no nucleus-less
//      fragments), an isolated nucleus grabs its whole mask blob, and touching
//      cells stop at the ridge between them.
//
// Keep a cell iff area >= CELL_MIN_SIZE px AND raw p99 >= SIGNAL_BG_MULT*bg in
// BOTH channels. Optional nucleus subtraction. Returns the kept-ROI count.
// ------------------------------
function buildAutoCytosolRoi(m1, m2, bg1, bg2) {
    ...
    // Nucleus seed points (bbox centres, in pixels) — collected before we reuse
    // the ROI Manager for the cytosol ROIs.
    roiManager("reset");
    selectWindow("Nuc_Mask");
    run("Analyze Particles...", "size=" + NUC_MIN_AREA + "-Infinity pixel add");
    nNuc = roiManager("count");
    if (nNuc == 0) { print("  WARN: no nuclei — full image"); closeAutoMaskWindows(); return 0; }
    seedX = newArray(nNuc); seedY = newArray(nNuc);
    for (i = 0; i < nNuc; i = i + 1) {
        roiManager("Select", i);
        getSelectionBounds(bx, by, bw, bh);
        seedX[i] = round(bx + bw/2);
        seedY[i] = round(by + bh/2);
    }
    roiManager("reset");

    makeCellMask(cellSrc);                 // Cell_Mask (solid: blur + threshold + close + fill)
    imageCalculator("OR", "Cell_Mask", "Nuc_Mask");   // ensure every nucleus is inside the mask
    splitCellsByNucleusVoronoi();          // Cells_Split (cell mask cut between nuclei)

    // Grow each nucleus = grab its territory from Cells_Split by flood-fill.
    ...
    for (i = 0; i < nNuc; i = i + 1) {
        ...
        run("Select None");
        if (getPixel(seedX[i], seedY[i]) == 0) continue;   // seed not on a cell (ridge/bg)
        doWand(seedX[i], seedY[i], 0.0, "8-connected");    // the cell territory of nucleus i
        if (selectionType() == -1) continue;

        // Build this cell's cytosol mask from the flood-fill selection.
        getDimensions(W, H, ch, sl, fr);
        newImage("cell_tmp", "8-bit black", W, H, 1);
        run("Restore Selection");          // the doWand selection on cell_tmp
        setColor(255); fill();
        run("Select None");
        ...
        if (cytoMean == 0) { closeIfOpen("cell_tmp"); continue; }
        run("Create Selection");           // one selection for this cell's cytosol
        roiManager("Add");
        cytoIdx = roiManager("count") - 1;
        closeIfOpen("cell_tmp");

        // keep filter: area + raw p99 >= MULT*bg in both channels.
        ...
        keep = (nPix >= CELL_MIN_SIZE) && (p1 >= minSig1) && (p2 >= minSig2);
        if (keep) {
            roiManager("Select", cytoIdx);
            roiManager("Rename", "cell_" + (kept+1));
            kept = kept + 1;
            print("  kept nucleus #" + (i+1) + ": area=" + nPix
                + " p99(" + m1 + ")=" + p1 + " (>=" + minSig1 + ")"
                + " p99(" + m2 + ")=" + p2 + " (>=" + minSig2 + ")");
        } else {
            roiManager("Select", cytoIdx);   // non-kept = last entry → safe to delete
            roiManager("Delete");
        }
    }

    closeAutoMaskWindows();
    print("  auto ROIs kept: " + kept + " of " + nNuc + " nuclei");
