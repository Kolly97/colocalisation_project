...
| v1.0.0 | Field tuning (Fiji): **merged `AUTO_EXCLUDE_NUC` + `MANUAL_EXCLUDE_NUC` → one `EXCLUDE_NUC`** (one checkbox governs both modes), `NUC_MIN_AREA` 100 → **5000 px** (only large, real nuclei seed a cell — drops debris and tiny mitotic fragments), `SIGNAL_BG_MULT` 2 → **1.5** (keep weak-but-real HA568 cells), `GATE_RING_UM` 4 → **8 µm** (ring scales with the large cells). *(The shipped file initially carried the v0.14.2 header/version string by copy-paste; corrected to 0.14.3.)* |
