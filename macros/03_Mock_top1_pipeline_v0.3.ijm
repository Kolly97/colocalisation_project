...
//
// Changes vs V0.1:
//  - FIX:   getHistogram quirk for 16-bit, full-range histograms.
//           IJM does NOT populate `values[]` in that case
//           (memory optimisation). We now use bin index `i`
//           directly as the pixel value (bin width = 1).
//  - NEW:   Startup mode selector: "filename" vs "dialog".
//  - NEW:   Startup config dialog (in dialog mode) for
//           CELL_LINE / MARKERS / ANALYSE_COMBI / TIMEPOINTS.
//  - NEW:   Per-image dialog asks timepoint + C1 + C2.
//  - NEW:   Filename-mode parse failure → ask skip vs dialog.
//  - IMPR:  Cell mask uses Enhance Contrast + scaled Gaussian
//           (sigma in microns, much smoother on hi-res images).
//  - IMPR:  Nucleus mask uses morphological closing
//           (Dilate → Fill Holes → Erode) so internal holes
//           with thin boundary gaps still get filled.
// ==========================================================
